    package com.vitcode.iprayertimes.easypermissions;
    ...
    public class EasyPermissions {
        ...
        public static void requestPermissions(final Object object, String rationale, @StringRes int positiveButton, @StringRes int negativeButton, final int requestCode, final String... perms) {
            ...
            if (shouldShowRationale) {
                new AlertDialog.Builder(getActivity(object)).setMessage((CharSequence) rationale).setPositiveButton(positiveButton, (DialogInterface.OnClickListener) new DialogInterface.OnClickListener() {
            ...
            for (Method method : object.getClass().getDeclaredMethods()) {
                if (method.isAnnotationPresent(AfterPermissionGranted.class) && ((AfterPermissionGranted) method.getAnnotation(AfterPermissionGranted.class)).value() == requestCode) {
                    if (method.getParameterTypes().length > 0) {
                        String valueOf = method.getName();
                        throw new RuntimeException(valueOf.length() != 0 ? "Cannot execute non-void method ".concat(valueOf) : "Cannot execute non-void method ");
                    }
                    try {
